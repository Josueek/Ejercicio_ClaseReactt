...
const App = () => {
    ...
    const [matAlumno, setMateriaAlumno] = useState('');
    const [carnetAlumno, setCarnetAlumno] = useState('');
    ...
    const agregarCliente = () => {
        ...
        const nuevoCliente = { id: clientes.length + 1, nombre: nombre, fechaReserva: fechaReserva, matAlumno: matAlumno, carnetAlumno: carnetAlumno };
    ...
    const eliminarCliente = (id) => {
        ...
        // Actualiza los IDs de los clientes restantes
        const clientesConIDsActualizados = clientesActualizados.map((cliente, index) => ({ ...cliente, id: index + 1 }));
        // Actualiza el estado de los clientes con los IDs actualizados
        setClientes(clientesConIDsActualizados);
    };
    return (
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        ...
                        <TextInput
                            ...
                            placeholder="Materia"
                            value={matAlumno}
                            onChangeText={setMateriaAlumno}
                        />
                        <TextInput
                            style={styles.input}
                            placeholder="Carnet"
                            value={carnetAlumno}
                            onChangeText={setCarnetAlumno}
                        />
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    ...
                    >
                        ...
                        <Text style={styles.matAlumno}>{item.matAlumno}</Text>
                        <Text style={styles.carnetAlumno}>{item.carnetAlumno}</Text>
                        <Text style={styles.clienteFecha}>
                            Fecha de Reserva: {item.fechaReserva.toDateString()}
                        </Text>
                        {/* Botón para eliminar el registro del cliente */}
                        <TouchableOpacity
